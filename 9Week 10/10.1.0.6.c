...
int main(void)
{
  ...
  base_0 = allocate_step(board_0);
  
  make_tree(s_0, base_0);
  
  print_board(s_0->board);
}
...
Step *moves_from(Step *s, int i, int j, Step *base)
{
  ...
  if(is_movable_up(s->board, i, j) == 1)
  {
    ...
    {
      board_copy(move_up(s->board, i, j), t->board);
      insert_step(t);
      t->previous_move = s;
      return(t);
    }
  ...
  if(is_movable_right(s->board, i, j) == 1)
  {
    ...
    {
      board_copy(move_right(s->board, i, j), t->board);
      insert_step(t);
      t->previous_move = s;
      return(t);
    }
