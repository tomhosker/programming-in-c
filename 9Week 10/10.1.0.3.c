...
int main(void)
{
  ...
  print_board(s_0->next_in_register->next_in_register->next_in_register->board);
...
Step *search_tree(Step *s, char array[BOARD_HEIGHT][BOARD_WIDTH])
{
  ...
  while((s->next_in_register != NULL) && (board_compare(s->board, array) != 0))
  {
